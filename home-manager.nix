...
let
  ...
  persistentStorages = lib.pipe config.home.persistence [
    (builtins.mapAttrs (_: persistentStorage: persistentStorage // {
      mkDirCfg = getHomeDirCfg {
        inherit storage pkgs;
        inherit (config.home) homeDirectory;
      };
    }))
    (lib.attrsets.filterAttrs (_: persistentStorage: persistentStorage.enable))
  ];
